    ...
    public class UnitTest1
    {
        ...
        public async Task TestMethod1()
        {
            ...
            using (var session = docStore.OpenSession(new SessionOptions() { Timeout = 30 }))
            {
                ...
                var query = session
                    .Query<TestData>()
                    .Where(td => td.InnerTestDatas.Any(itd => itd.InnerTestDataId == "innertestdata_1"));
                var result = await query.ToListAsync();
                Assert.Equal(1, result.Count);
            }
        }

        private DocumentStore GetDocumentStore(string userConnectionStringBase, string dbName)
        {
            string connectionString = $"{userConnectionStringBase};Database={dbName}";
            ...
            var docStore = DocumentStore.For(_ =>
            {
                ...
                _.Connection($"{userConnectionStringBase};Database={dbName}");
